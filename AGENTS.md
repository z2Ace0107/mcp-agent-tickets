...
```
v5.1 — Phase 1-4 已完成 ✅
  ✓ Phase 1: 种子数据 / 知识库 / 2 新工具 / TOOL_CN_MAP
  ✓ Phase 2: Context Engine — 三层组装 / 回合压缩
  ✓ Phase 3: Agent 稳定性 — TOOL_CALL_LIMITS / 动态移除 / 每轮上限 / 工具名验证
  ✓ Phase 4: 评测重写 — 54 题（A15/B20/C15/D4）+ required/optional 双列表 + 任务完成度

➡ 下一步: Phase 5 — 前端优化（P2 优先级）
```
...
| 文件 | 职责 | 状态 |
|------|------|:---:|
| `backend/agent_loop.py` | Agent 核心循环 + Context Engine + 限频分级 | Phase 3 完成 ✅ |
...
| `eval/test_queries.json` | 54 题评测集（required/optional 双列表） | Phase 4 完成 ✅ |
| `eval/judge.py` | 评测框架（必要工具覆盖率 + 任务完成度） | Phase 4 完成 ✅ |
