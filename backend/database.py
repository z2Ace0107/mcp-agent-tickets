...
def _seed_tickets(conn: sqlite3.Connection) -> None:
    ...
    tickets = [
        ...
        # ===== v5.1 根因链工单 (6条) — 同一设备反复故障的追溯路径 =====
        # 链1: CNC-MC-003 主轴故障链 (WO-001→028→029)
        ("WO-20260517-028", "CNC加工中心主轴轴承再次异响 — 更换后仅3周复发",
         "设备故障", "已解决", "紧急",
         "张建国", "2026-05-17",
         "设备编号：CNC-MC-003 | 型号：DMG MORI DMU 50\n"
         "故障现象：5月17日早班，主轴转速升至7500rpm时再次出现尖锐异响，振动值2.8mm/s。\n"
         "前次维修记录：4月28日更换主轴轴承（SKF 7014 ACD/P4A），仅3周后复发。\n"
         "初步判断：轴承安装预紧力不当或润滑管路存在微堵。",
         "拆检发现轴承外圈与轴承座配合间隙偏大（0.012mm，标准≤0.005mm），安装时未使用专用压裝工具导致轴承座微变形。"
         "重新铰削轴承座孔并镶套修复，使用专用工具压装新轴承（SKF 7014 ACD/P4A），预紧力按标准值34±2Nm。"
         "清洗润滑管路并更换滤芯。运行72h后振动值稳定在0.7mm/s。",
         "EQP-001", "LINE-001", "MAT-003"),

        ("WO-20260521-029", "CNC加工中心加工精度超差 — 主轴箱体温度异常",
         "设备故障", "已解决", "紧急",
         "张建国", "2026-05-21",
         "设备编号：CNC-MC-003 | 型号：DMG MORI DMU 50\n"
         "故障现象：最近2天加工壳体轴承孔圆度超差（要求≤0.005mm，实测0.012-0.018mm），主轴箱体温度偏高（52℃，正常≤40℃）。\n"
         "回溯：此设备4月28日和5月17日两次更换主轴轴承，加工精度问题在最后一次维修后出现。\n"
         "根因分析：润滑系统主油路滤芯在第二次维修时发现微堵，但仅清洗未更换；实际油路末端仍有残留碎屑导致供油不足，轴承预紧力缓慢衰减。",
         "更换润滑系统主油路滤芯（非仅清洗），反向冲洗所有润滑管路，更换全部润滑油（ISO VG 32主轴专用油）。"
         "重新校准主轴动平衡至G1级，轴承预紧力恢复至34Nm。加工精度恢复至圆度≤0.004mm，主轴温度降至37℃。"
         "建立润滑系统每500h强制清洗+滤芯更换制度，替代原'视情清洗'。",
         "EQP-001", "LINE-001", None),

        # 链2: EQP-008 冲压线安全光幕故障链 (030→031→032)
        ("WO-20260503-030", "冲压车间安全光幕频繁误触发 — 3号线停线",
         "安全隐患", "已解决", "高",
         "李明辉", "2026-05-03",
         "设备编号：HP-02 | 型号：315T四柱液压机 | 产线：3号线\n"
         "问题：5月3日早班起，冲压线安全光幕在无人员进入时频繁触发急停（上午2小时内触发7次），"
         "每次复位后能恢复但影响节拍，产线有效运行时间仅65%。\n"
         "初步排查：光幕发射端/接收端表面有少量油雾凝结，用无尘布擦拭后恢复。",
         "清洁光幕发射端和接收端光学窗口，调整光幕安装角度避免冲压油雾直接喷溅。"
         "光幕恢复正常，误触发率降至0。建立每班次清洁光幕光学窗口的点检制度。",
         "EQP-006", "LINE-001", None),

        ("WO-20260505-031", "安全光幕再次误触发 — 更换接线端子后间歇性故障",
         "安全隐患", "已解决", "紧急",
         "李明辉", "2026-05-05",
         "设备：HP-02 315T液压机安全光幕系统（与WO-20260503-030同设备）\n"
         "问题：5月5日下午再次出现误触发，清洁光幕后仍无法彻底解决。\n"
         "深入排查：光幕接收端信号线在电气柜内接线端子处有氧化发黑痕迹，"
         "接触电阻从正常<0.1Ω升至3-5Ω，导致信号偶尔丢失被PLC判断为遮光。",
         "更换光幕接收端信号线接线端子（原为普通铜端子，更换为镀金弹簧端子）。"
         "对所有冲压线安全光幕的电控柜接线做预防性检查，紧固并涂抹防氧化导电膏。"
         "误触发问题彻底解决，后续2周无复发。",
         "EQP-006", "LINE-001", None),

        ("WO-20260512-032", "冲压件毛刺超标 — 模具错位导致大批返工",
         "质量异常", "已解决", "高",
         "王桂芳", "2026-05-12",
         "产品：壳体冲压件 | 产线：3号线 | 模具编号：DIE-SH-017\n"
         "问题：5月12日质检巡检发现冲压件断面毛刺高度平均0.15mm（标准≤0.05mm），"
         "当天已产出的220件全部需返工去毛刺。\n"
         "根因追溯：调取冲压机PLC日志发现，5月11日夜班期间安全光幕共触发急停4次，"
         "其中1次急停发生在模具闭合行程中途。液压系统压力冲击导致上模微量位移（约0.08mm），"
         "上下模间隙从0.04mm扩大至0.12mm。\n"
         "根本原因：安全光幕信号干扰（WO-030/031）引发的急停连锁效应。",
         "重新校正模具闭合位置精度，上下模间隙恢复至0.04mm。"
         "在PLC程序中增加急停后自动检测模具位置的功能，偏差超过0.03mm自动报警并锁定。"
         "建立光幕触发与模具状态的关联巡检制度。毛刺高度恢复至≤0.05mm。",
         "EQP-006", "LINE-001", None),

        # ===== v5.1 跨部门联动工单 (8条) =====
        ("WO-20260508-033", "3号生产线设备故障导致全线停产 — 影响当日壳体验收",
         "设备故障", "已解决", "紧急",
         "张建国", "2026-05-08",
         "关联设备：CNC-MC-003 | 影响产线：LINE-001 3号生产线\n"
         "事故描述：5月8日10:15起CNC-MC-003主轴冷却系统报警停机，3号生产线全停。\n"
         "影响范围：当日壳体精加工计划产量280件无法完成，已加工的120件半成品需等设备修复后继续。\n"
         "连锁影响：壳体缺件将导致次日装配线（LINE-002）液压油缸总成待料停产。\n"
         "紧急措施：调动2号线CNC设备临时分担部分加工任务，加班至22:00补产。",
         "CNC-MC-003于当日14:30修复，3号线恢复运行。2号线+3号线联合加班完成当日全部280件壳体加工。"
         "建立关键设备故障时的跨线产能调配预案，设定壳体安全库存从2天提升至3天。",
         "EQP-001", "LINE-001", None),

        ("WO-20260509-034", "液压油缸装配线待料停产 — 壳体供应中断",
         "物料短缺", "已关闭", "紧急",
         "陈晓东", "2026-05-09",
         "关联：WO-20260508-033 设备故障的连锁影响\n"
         "问题：5月9日早班，装配线（LINE-002）因壳体库存耗尽而停产待料。"
         "原计划5月8日完成的壳体加工因CNC设备故障延迟，虽然当日加班完成，但物流交接窗口错过，"
         "壳体次日9:30才送达装配线，导致装配线停工2小时。\n"
         "物料编号：MAT-003 INA滚子轴承也因安全库存不足（仅剩2件，装配线需5件/天），同步出现短缺预警。",
         "紧急调度壳体物料，装配线10:00恢复运行。将轴承安全库存从2天调整为5天，"
         "壳体从3天调整为4天。建立关键物料消耗与上游设备状态的联动预警机制。",
         None, "LINE-002", "MAT-003"),

        ("WO-20260506-035", "外协热处理批次质量问题 — 导致后续机加工序刀具异常磨损",
         "质量异常", "处理中", "高",
         "王桂芳", "2026-05-06",
         "外协厂：昆山鑫泰热处理 | 物料：40Cr调质钢棒材（MAT-001）\n"
         "问题：外协热处理返回的40Cr棒材在后续CNC车削时刀具寿命从正常200件/刀骤降至40件/刀，"
         "切削力异常增大。检测发现棒材表面硬度HRC 35-38（要求HRC 28-32），淬火后未按要求回火。\n"
         "影响：已领用的50根棒材需全部隔离，已加工的15件传动轴需金相检测确认是否可用。\n"
         "上游追溯：这批40Cr钢材来自武汉钢铁（供应商），需同步确认原材料本身无问题。",
         "", None, None, "MAT-001"),

        ("WO-20260507-036", "涂装车间环境超标 — 精密加工车间投诉空气颗粒物增多",
         "环境监测", "处理中", "高",
         "刘红梅", "2026-05-07",
         "监测点：涂装车间废气排放口DA-002 | 关联投诉：精密加工车间\n"
         "问题：精密加工车间反映最近3天加工区域空气颗粒物（PM2.5）从正常的≤15μg/m³升至45μg/m³，"
         "怀疑影响精密主轴加工表面质量（磨削工序对环境洁净度敏感）。\n"
         "排查：涂装车间VOCs废气处理系统（活性炭吸附AC-02）的处理效率从正常的92%降至74%，"
         "部分未处理完全的含颗粒物废气通过厂房屋顶排风口飘散至临近的精密加工车间。",
         "", "EQP-010", None, None),

        ("WO-20260415-037", "供应商40Cr钢材批次更换导致加工参数全部失效",
         "工艺问题", "已解决", "高",
         "张建国", "2026-04-15",
         "供应商：鞍钢集团（原武汉钢铁） | 物料：45#调质钢棒材（MAT-005替代原40Cr）\n"
         "背景：因武汉钢铁40Cr（MAT-001）交期延迟，采购部紧急切换为鞍钢45#钢（MAT-005）替代，"
         "但两种材料的切削工艺参数差异较大（45#硬度低约15%，最佳切削速度高20%）。\n"
         "问题：加工车间继续使用40Cr参数加工45#钢，导致表面粗糙度超差、刀具异常磨损。\n"
         "影响范围：涉及3种传动轴产品，共约500件在制品需重新调整参数后加工。",
         "工艺工程师紧急编制45#钢临时工艺参数表（切削速度从180m/min调整至220m/min，"
         "进给量从0.15mm/r调整至0.20mm/r）。已加工500件逐件复查，合格率92%，不合格品返工。"
         "在ERP系统中建立物料替代时的工艺参数联动提醒规则。",
         None, None, "MAT-005"),

        ("WO-20260516-038", "成品库恒温恒湿系统长时间故障 — 精密轴承批量生锈索赔",
         "质量异常", "待处理", "紧急",
         "陈晓东", "2026-05-16",
         "关联：WO-20260514-026 成品库恒温恒湿系统故障的后续\n"
         "问题：5月14日凌晨恒温恒湿仓故障导致湿度骤升至82%，虽当日修复，但已造成部分精密件受潮。"
         "5月16日客户（日本NSK）反馈收到的一批精密轴承（SKF 7014 ACD/P4A，共120件）"
         "中发现8件外圈出现点状锈蚀，要求整批退货并索赔￥42万。\n"
         "根因：该批轴承在5月14日湿度异常期间位于仓库靠近空调出风口的区域，凝露最严重。\n"
         "售后：需紧急补货120件并承担往返运费+客户停线损失约￥8万。",
         "", None, None, "MAT-003"),

        ("WO-20260518-039", "空压机AC-02跳闸导致全厂气压波动 — 多台气动设备连锁故障",
         "设备故障", "处理中", "紧急",
         "李明辉", "2026-05-18",
         "设备：AC-02 Atlas Copco GA90 空压机（与WO-20260508-003同设备）\n"
         "影响链：\n"
         "1. AC-02跳闸 → 全厂气压从6.0bar骤降至3.8bar → 1号机单独供气仅5.2bar\n"
         "2. 影响气动设备：装配线气动扳手TQ-03（WO-20260512-024，扭力不足）、"
         "AGV搬运车气动夹具（AGV-05，工件滑落3次）、注塑机气动脱模机构（IM-06，脱模延迟）\n"
         "3. 最终影响：装配线产能降至40%，注塑机产出效率下降25%\n"
         "根因：AC-02变频器散热风扇故障导致IGBT模块过热保护，需更换变频器。",
         "", "EQP-003", "LINE-002", None),

        ("WO-20260514-040", "供应商物流包装变更导致来料批量损伤 — 装配线被迫停用",
         "物料短缺", "待处理", "中",
         "陈晓东", "2026-05-14",
         "关联：WO-20260515-027 供应商来料包装破损率升高的问题\n"
         "供应商：太钢不锈 | 物料：304冷轧不锈钢板（MAT-004）\n"
         "问题：太钢不锈近期将木箱包装改为纸箱+钢带捆扎以降低成本，但新包装在运输中抗挤压能力不足，"
         "最近3车次到货时板材边角磕碰变形率达28%（正常≤3%），变形板材无法进入精密冲压工序。\n"
         "影响：装配线所需的不锈钢冲压件库存仅够3天，若下一批来料仍有问题将导致停产。",
         "", None, None, "MAT-004"),

        # ===== v5.1 模糊描述工单 (5条) — Agent 需主动查表关联 =====
        ("WO-20260519-041", "3号机早上开机有异响 — 不确定是什么问题",
         "设备故障", "待处理", "中",
         "", "2026-05-19",
         "报告人：操作工李师傅（口述记录）\n"
         "现象：今天早上8点开机时听到3号机方向有'咔嗒咔嗒'的声音，大概持续了十几秒就没了，"
         "现在机器还在跑，但不放心。具体哪台机器没看清，可能是CNC那边的。",
         "", None, None, None),

        ("WO-20260520-042", "这批产品好像不太对劲 — 表面摸起来手感不一样",
         "质量异常", "待处理", "中",
         "", "2026-05-20",
         "报告人：质检员小周\n"
         "产品：壳体精加工件 | 产线：3号线\n"
         "现象：今天抽检时感觉这批壳体表面手感跟平时不一样，有点涩，不像正常那样光滑。"
         "量了尺寸和粗糙度都在范围内，但就是感觉不对。怀疑是不是冷却液有问题或者砂轮磨损了。",
         "", None, "LINE-001", None),

        ("WO-20260521-043", "生产线突然停了 — 面板上好几个红灯在闪",
         "设备故障", "待处理", "紧急",
         "", "2026-05-21",
         "报告人：线长王师傅\n"
         "产线：3号线（LINE-001）\n"
         "现象：刚才产线突然停了，操作面板上好几个报警灯都在闪，不知道哪个设备先触发的。"
         "液压机那边压力表好像也归零了。复位了一次没用，第二次才重新启动。"
         "现在是跑起来了但感觉节拍比平时慢。",
         "", None, "LINE-001", None),

        ("WO-20260522-044", "最近废品率好像高了 — 但不知道怎么查",
         "质量异常", "待处理", "中",
         "", "2026-05-22",
         "报告人：车间主任赵工\n"
         "问题：这周感觉废品区堆的东西比以前多，问了一圈没人知道原因。"
         "可能是某个工序出了问题，也可能是材料批次不一样。"
         "帮忙查一下这周的不良率数据，看看是哪个工序、哪个产品出问题最多。",
         "", None, None, None),

        ("WO-20260523-045", "仓库温度比平时高 — 不知道空调是不是又坏了",
         "环境监测", "待处理", "中",
         "", "2026-05-23",
         "报告人：库管员刘姐\n"
         "地点：成品库2号恒温恒湿仓\n"
         "现象：今天进仓库感觉比平时闷热，温度计显示26℃（平时设定22℃），湿度没注意看。"
         "空调外机声音好像比平时大。上次5月14号刚修过空调（WO-20260514-026），"
         "不知道是不是又坏了。",
         "", None, None, None),

        # ===== v5.1 方案对比工单 (4条) — 同一问题有多个解决方案 =====
        ("WO-20260410-046", "冲压模具寿命短 — D2钢冲头频繁崩刃",
         "工艺问题", "已解决", "高",
         "李明辉", "2026-04-10",
         "模具编号：DIE-SH-017 | 加工材料：304不锈钢板2.0mm厚\n"
         "问题：冲压壳体用的D2钢冲头平均寿命仅8000冲次（行业标准≥20000），频繁崩刃导致停机换模，"
         "每次换模需停产45分钟，月均换模12次。\n"
         "方案A（已实施）：更换为粉末冶金高速钢PM-HSS冲头，寿命提升至22000冲次，"
         "单件模具成本从￥850升至￥2100。\n"
         "经济性：每月减少换模6次，节省换模时间4.5h/月，年化节省约￥18,600但模具成本年增￥15,000，净节省￥3,600/年。",
         "方案A已实施：更换PM-HSS冲头。同时改进冲裁间隙（从0.08mm调整至0.10mm）和润滑方案（从机油改为极压润滑脂），"
         "冲头寿命最终达到25000冲次。建立模具寿命台账，按冲次数预警替换。",
         "EQP-006", "LINE-001", "MAT-004"),

        ("WO-20260412-047", "磷化膜结晶粗大问题 — 三种调整方案对比",
         "工艺问题", "已解决", "中",
         "刘红梅", "2026-04-12",
         "关联：WO-20260513-025 同类型磷化线问题\n"
         "问题：磷化线PL-02槽液参数偏移导致磷化膜结晶粗大（晶粒从正常的3-8μm增大至15-25μm），"
         "技术组提出三种调整方案：\n"
         "方案A：加NaOH中和游离酸（成本最低，见效快但易反弹）\n"
         "方案B：部分更换槽液+补充添加剂（成本中等，效果稳定）\n"
         "方案C：全部更换新槽液（成本最高，效果最佳但停产1天）\n"
         "选择方案B：更换槽液30%+补加促进剂至标准浓度3.5点，总费用￥3,200，半天完成。",
         "方案B实施：更换槽液30%（约2400L），补加促进剂至标准浓度，游离酸度从1.8点恢复至1.0点。"
         "磷化膜晶粒恢复至4-7μm正常范围。建立槽液参数每日检测+SPC控制图，超出控制上限提前预警。",
         None, None, "MAT-006"),

        ("WO-20260418-048", "注塑机温控系统PID改造 — 机械式温控 vs 智能PID vs 模糊PID",
         "设备故障", "已解决", "中",
         "张建国", "2026-04-18",
         "设备：IM-06 海天MA3200注塑机\n"
         "背景：WO-20260506-002 温控系统失控事件后，评估三种温控改造方案：\n"
         "方案A：保留原机械式温控，仅更换热电偶和继电器（费用￥1,500，精度±3℃）\n"
         "方案B：升级为智能PID温控模块（费用￥8,500，精度±0.5℃，需定制安装）\n"
         "方案C：升级为模糊PID+自整定模块（费用￥28,000，精度±0.2℃，含远程监控）\n"
         "选择方案B：综合考虑成本与精度需求，方案C的高精度对注塑加工过度冗余。",
         "方案B实施：安装欧姆龙E5CC智能PID温控器，更换K型热电偶和固态继电器。"
         "PID参数经3轮自整定优化后，温度控制精度从±3℃提升至±0.5℃，过冲量从15℃降至3℃。"
         "年化减少因温控问题导致的报废约￥12,000，投资回收期约8.5个月。",
         "EQP-002", None, None),

        ("WO-20260422-049", "AGV搬运车导航升级方案评估 — SLAM优化 vs 磁条引导 vs QR码定位",
         "设备故障", "已解决", "高",
         "李明辉", "2026-04-22",
         "设备：AGV-05 海康威视MR-Q3-300\n"
         "背景：WO-20260420-004 AGV撞坏物料架事件后，评估三种导航方案：\n"
         "方案A：优化现有激光SLAM参数+禁停区标识（费用￥2,000，当天完成）\n"
         "方案B：关键区域铺设磁条引导作为SLAM辅助（费用￥35,000，施工3天，准确率99.5%）\n"
         "方案C：全仓库部署QR码定位系统（费用￥180,000，施工2周，准确率99.9%）\n"
         "选择方案A+B：普通区域用方案A优化后的SLAM，转弯/狭窄通道等关键区域铺设磁条辅助。",
         "方案A+B实施：重新标定AGV-05激光SLAM参数，转弯通道安装磁条引导+RFID位置校验标签。"
         "通道宽度下限调整为2.0m。额外铺设地面反光标识。总费用￥37,000，施工3天完成。"
         "后续3个月AGV导航偏差率从事故前的0.5%降至0.05%，零碰撞记录。",
         "EQP-004", None, None),
